...
class VAEWMAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths), total=len(image_paths), desc='VAE Attacking' , unit='image'):
...
class GaussianBlurAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths), total=len(image_paths), unit='image', desc='Gaussian Blur Attacking'):
...
class GaussianNoiseAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths), total=len(image_paths), unit='image', desc='Gaussian Noise Attacking'):
...
class BM3DAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths), total=len(image_paths), unit='image', desc='BM3D Attacking'):
            img = Image.open(img_path).convert('RGB')
...
class JPEGAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths), total=len(image_paths), unit='image', desc='JPEG Attacking'):
...
class BrightnessAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths), total=len(image_paths), unit='image', desc='Brightness Attacking'):
            img = Image.open(img_path)
...
class ContrastAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths), total=len(image_paths), unit='image', desc='Contrast Attacking'):
            img = Image.open(img_path)
...
class RotateAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths), total=len(image_paths), unit='image', desc='Rotate Attacking'):
            img = Image.open(img_path)
...
class ScaleAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths), total=len(image_paths), unit='image', desc='Scale Attacking'):
            img = Image.open(img_path)
            w, h = img.size
...
class CropAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths):
        for (img_path, out_path) in tqdm(zip(image_paths, out_paths), total=len(image_paths), unit='image', desc='Crop Attacking'):
            img = Image.open(img_path)
            w, h = img.size
...
class DiffWMAttacker(WMAttacker):
    ...
    def attack(self, image_paths, out_paths, return_latents=False, return_dist=False):
        with torch.no_grad():
            ...
            for (img_path, out_path), prompt in tqdm(zip(zip(image_paths, out_paths), prompts), total=len(image_paths), unit='image', desc='Diffuse Attacking'):
                img = Image.open(img_path)
