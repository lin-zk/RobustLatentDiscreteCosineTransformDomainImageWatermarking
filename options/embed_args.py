...
class EmbedOptions():
	def __init__(self):
		self.parser = argparse.ArgumentParser(description="Options for embedding images with watermark.")
	...
	def initialize(self):
		...
		self.parser.add_argument(
			"--fingerprint_mode",
			type=str,
			choices=["random", "fix", "certain"],
			default="random",
			help="Mode for generating fingerprints: random, fix, or certain.",
		)
		self.parser.add_argument(
			"--fingerprint_content",
			type=str,
			help="Content for the fingerprint when mode is 'certain'.",
		)
		self.parser.add_argument(
			"--image_resolution",
			type=int,
	...
	def parse(self):
		...
		self.opt.output_dir = self.opt.data_dir  # 设置输出目录为数据目录
		...
		file_name = os.path.join(self.opt.expr_dir, 'embed_opt.txt')
